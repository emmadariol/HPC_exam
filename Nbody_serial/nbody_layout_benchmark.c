// Force-only benchmark: AoS vs SoA memory layout (OpenMP, no MPI).
...
typedef struct particle_aos_s  // array of structures: all data of one particle together
{
  dtype x, y, z;
  dtype vx, vy, vz;
  dtype ax, ay, az;
...
typedef struct particles_soa_s  // structure of arrays: one array per component
{
  size_t n;
  dtype *x, *y, *z;
  dtype *vx, *vy, *vz;
...
typedef enum layout_e
{
  LAYOUT_SOA,
  LAYOUT_AOS,
  LAYOUT_BOTH  // run SoA and then AoS
} layout_t;

typedef enum rsqrt_mode_e
{
  RSQRT_EXACT,
  RSQRT_APPROX  // float estimate + two Newton-Raphson steps
} rsqrt_mode_t;

static _Noreturn void die(const char *fmt, ...)  // print an error and exit
{
  va_list args;
  va_start(args, fmt);
...
static const char *option_value(int *i, int argc, char **argv,  // accepts both --key value and --key=value
                                const char *key)
{
...
static inline dtype invsqrt_force(dtype r2, rsqrt_mode_t mode)  // 1/sqrt(r2), exact or approximate
{
  if (mode == RSQRT_EXACT)
    return (dtype)1.0 / dtype_sqrt(r2);
  else
  {
    dtype x = (dtype)(1.0f / sqrtf((float)r2));  // float estimate
    const dtype half = (dtype)0.5;
    const dtype three_halves = (dtype)1.5;
    x = x * (three_halves - half * r2 * x * x);  // first Newton-Raphson step
    x = x * (three_halves - half * r2 * x * x);  // second Newton-Raphson step
    return x;
  }
}

static void *checked_aligned_alloc(size_t nbytes)  // aligned allocation, size rounded up
{
...
static void particles_soa_allocate(particles_soa_t *p, size_t n)  // allocate the nine SoA arrays
{
...
static particle_aos_t *particles_aos_allocate(size_t n)  // one zeroed array of particle structs
{
...
static void read_particles(const char *path, particles_soa_t *soa,  // read the file once and fill both layouts
                           particle_aos_t **aos_out)
{
  ...
  for (i = 0u; i < n; ++i)  // one float record per particle
  {
    float rec[NBODY_BINARY_COMPONENTS];
    checked_fread(rec, sizeof rec[0], NBODY_BINARY_COMPONENTS, fp, path,
                  "particle record");
    soa->x[i] = aos[i].x = (dtype)rec[0];  // same values in SoA and AoS
...
// ===========================================================================
// FORCE LOOP, SoA LAYOUT: [OpenMP] + [SIMD]
// ===========================================================================
static void compute_soa(particles_soa_t *p, dtype g, dtype mass, dtype eps,  // force loop on the SoA layout
...
#pragma omp parallel for schedule(static)  // [OpenMP] each thread owns different targets
  for (i = 0u; i < p->n; ++i)
  {
    const dtype xi = p->x[i], yi = p->y[i], zi = p->z[i];  // target position
    dtype ax = (dtype)0.0, ay = (dtype)0.0, az = (dtype)0.0;
    size_t j;
#pragma omp simd reduction(+ : ax, ay, az)  // [SIMD] vectorisation hint for the inner loop
    for (j = 0u; j < p->n; ++j)
    {
      const dtype dx = p->x[j] - xi;  // d = r_j - r_i (self-pair gives d = 0)
...
// ===========================================================================
// FORCE LOOP, AoS LAYOUT: [OpenMP]
// ===========================================================================
static void compute_aos(particle_aos_t *p, size_t n, dtype g, dtype mass,  // same force loop on the AoS layout
...
#pragma omp parallel for schedule(static)  // [OpenMP] each thread owns different targets
  for (i = 0u; i < n; ++i)
  {
    ...
    for (j = 0u; j < n; ++j)
    {
      const dtype dx = p[j].x - xi;  // x, y, z read from the particle struct
...
static double checksum_soa(const particles_soa_t *p)  // sum of all accelerations, to compare the two layouts
{
  double sum = 0.0;
  size_t i;
#pragma omp parallel for reduction(+ : sum) schedule(static)  // [OpenMP]
  for (i = 0u; i < p->n; ++i)
...
static void run_one_layout(layout_t layout, particles_soa_t *soa,  // warm-ups, then time `repeats` force evaluations
                           particle_aos_t *aos, dtype g, dtype mass,
                           ...
                           rsqrt_mode_t rsqrt_mode, bool header)
{
  ...
  for (r = 0u; r < warmups; ++r)  // unrecorded runs
    compute_layout_once(layout, soa, aos, g, mass, eps, rsqrt_mode);

  t0 = omp_get_wtime();  // timed block
  for (r = 0u; r < repeats; ++r)
    compute_layout_once(layout, soa, aos, g, mass, eps, rsqrt_mode);
  elapsed = omp_get_wtime() - t0;
  checksum = layout == LAYOUT_SOA ? checksum_soa(soa) : checksum_aos(aos, soa->n);  // checksum of the forces just computed
  ...
  printf("%s,%zu,%d,%zu,%zu,%s,%.9g,%.9g,%.17g\n",
         ...
         ((double)repeats * (double)soa->n * (double)(soa->n - 1u)) /  // billions of pairs per second
           (elapsed * 1.0e9),
         checksum);
}
...
int main(int argc, char **argv)
{
  ...
  if (layout == LAYOUT_BOTH)  // both layouts on the same input
  {
